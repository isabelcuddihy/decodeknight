...
game = Chessboard(GUI=False, render_delay_sec=0.5, gs=6, num_colored_boxes=5)
...
def objectiveFunctionScore(grid):
    ...
    # Check that no adjacent squares have the same color
...
#                 shapePos, currentShapeIndex, currentColorIndex, grid, placedShapes, done = game.execute(command)
...
#         current_objective_function_score = objectiveFunctionScore(grid)
#         attempts_since_improvement = 0
# print(f"Solved with {len(placedShapes)} shapes")
########################################
