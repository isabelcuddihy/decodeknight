...
class Chessboard:
    def __init__(self, GUI=True, render_delay_sec=0.1, gs=6, num_colored_boxes=5):
        ...
        self.colors = ["#FFFFFF", "#000000"]  # White, Black

        # Mapping of color indices to color names (for debugging purposes)
        self.colorIdxToName = {0: "White", 1: "Black"}
    ...
    #Knight icon
        ...
        self.done = False
        self.knightPos = [0, 0]  # Starting position of the knightPos

        # Initialize grid with random colored boxes
        # Possible extension: obstacles
        #self._addObstacles(self.grid, num_colored_boxes)
        ...
        if GUI:
            ...
            self._refresh()
#Redfine excutes based on knight movements and fill in landing space
# add change color during placement
    def execute(self, command='e'):
        ...
        if command.lower() in ['e', 'export']:
            ...
            try:
                ...
                pass
            return self.knightPos,  self.currentColorIndex, self.grid, self.done
        if command.lower() in ['w', 'up']:
            ...
            self.knightPos[1] = min(self.gridSize - len(self.shapes[self.currentShapeIndex]), self.knightPos[1] + 1)
            ...
            self.knightPos[0] = min(self.gridSize - len(self.shapes[self.currentShapeIndex][0]), self.knightPos[0] + 1)
        ...
        # add change color during placement
            ...
            if self.canPlace(self.grid, self.shapes[self.currentShapeIndex], self.knightPos):
                self._placeKnight(self.grid, self.shapes[self.currentShapeIndex], self.knightPos, self.currentColorIndex)
                self.placedShapes.append((self.knightPos.copy(), self.currentColorIndex))
            ...
            if self.placedShapes:
                ...
                self._removeKnight(self.grid, self.shapes[lastShapeIndex], lastShapePos)
                if self.checkGrid(self.grid):
                    self.done = True
                else:
                    self.done = False
    ...
    def canPlace(self, grid, shape, pos):
        ...
        # Checks uf knight move is logical and within board dimensions.
        pass
    ...
    def _placeKnight(self, grid, shape, pos, colorIndex):
        for i, row in enumerate(shape):
            for j, cell in enumerate(row):
                if cell:
                    grid[pos[1] + i, pos[0] + j] = colorIndex

    def _removeKnight(self, grid, shape, pos):
        for i, row in enumerate(shape):
            for j, cell in enumerate(row):
                if cell:
    ...
    def _refresh(self):
        ...
        self._drawShape(self.screen, self.shapes[self.currentShapeIndex], self.colors[self.currentColorIndex], self.knightPos)

        pygame.display.flip()
        self.clock.tick(self.fps)
        time.sleep(self.sleeptime)

#potential extension: add obstacles that cannot be filled in and must be navigated around. These could be represented by a specific color index (e.g., -2) and would require additional logic in the canPlace and checkGrid methods to ensure that shapes cannot be placed on or adjacent to these obstacles.
    def _addObstacles(self, grid, num_boxes=5):
