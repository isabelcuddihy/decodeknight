...
class Chessboard:
    def __init__(self, GUI=True, render_delay_sec=0.1, grid_length=6, grid_width = 6, starting_knight_pos=(0, 0), obstacle_boxes=5):
        ...
        self.placedKnights = [(self.currentKnightPos[0][0], self.currentKnightPos[0][1])] # List to track previous knight positions in order (position)
    ...
    def _refresh(self):
        ...
        knight_rect = knight_text.get_rect(center=(
            self.currentKnightPos[0][0] * self.cellSize + self.cellSize // 2,
            self.currentKnightPos[0][1] * self.cellSize + self.cellSize // 2
        ))
